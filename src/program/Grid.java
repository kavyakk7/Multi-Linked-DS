...
public class Grid 
{
	...
	public Grid(int x, int y) 
	{
		...
		{
			System.out.println("Cannot instantiate a grid with the parameters provided! Quitting.");
			System.exit(1);
		}
	}
	...
	/**
	 * Get the node at a x,y coordinate
	 * 
	 */
	...
	/**
	 * Set the grid with provided dimensions
	 */
	public void fill() 
	{
		...
		{
			System.out.println("Invalid x coordinate. Choose a number between 0 and "+ (length - 1));
		...
		{
			System.out.println("Invalid y coordinate. Choose a number between 0 and "+ (width - 1));
		...
		{
			System.out.println("Invalid x coordinate. Choose a number between 0 and "+ (width - 1));
	...
	private void addCells(int f1, int f2, int s1, int s2, int t1, int t2) 
	{
		...
		if (!(n1.isInvalid() || n2.isInvalid()|| n1.getVal().getTag().equals("STR") || n2.getVal().getTag().equals("STR"))) 
		{
	...
	public void addRows() 
	{
		...
		{
			System.out.println("Invalid y coordinate. Choose a number between 0 and "+ (length - 1));
	...
	public void assignCell()
	{
		...
		while (!cont) 
		{
			...
			try {
				...
				{
					System.out
							.println("Invalid input. Either enter a number value or a string starting with \"");
				} 
				else
				{
					cont = true;
	...
	public void divideColumns()
	{
		...
		for (int i = 0; i < length; i++) 
		{
			divideCells(x1, i, y1, i, x2, i);
		}
	}

	private void insertRow(int i) 
	{
		// insert nodes from top down connecting previous to current to next
		if (i < 0 || i >= length)
		{
			// check validity
		...
		else
		{
			...
			for (int j = 0; j < width; j++) 
			{
				curr.setDown(temp.getDown());// set new column's right to next column
	...
	{
		// insert nodes basically from top down connecting previous to current to next
		if (i < 0 || i >= length) 
		{ 
			// check validity
			System.out.println("invalid index.");
		...
		else
		{
			...
			for (int j = 0; j < length; j++) 
			{
				curr.setRight(temp.getRight());// set new column's right to next column
	...
	public void insertColumn()
	{
		...
		while (x < 0 || x >= width) 
		{
			System.out.println("Index is out of bounds. Please enter a number between 0 and "+ (width - 1));
			x = scan.nextInt();
		}
		insertColumn(x);
		width++;
	...
	public void insertRow() 
	{
		...
		while (x < 0 || x >= length) 
		{
			System.out.println("Invalid index. Please select a number between 0 and "+ (length - 1));
			x = scan.nextInt();
		}
		insertRow(x);
		length++;
	...
	public void deleteRow() 
	{
		if (length <= 1) 
		{
			System.out.println("There is only one column. Cannot delete that one! Quitting....");
			return;
		} 
		else 
		{
			// need to go down the row to the left and connect each node to the node
